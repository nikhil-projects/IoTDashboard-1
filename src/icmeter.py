...
class Timeseries:
    ...
    def LoadAll(self, data_id, start=1199145600, stop = time.time(), period = 60 * 60 * 24 * 7):
...
class Ic(Timeseries):
    ...
    def GetDataPeriod(self, data_id, start = 1498720785, period = 60 * 60 * 24 * 7.0):
    ...
    def ImportAll(self, data_id = None, start = 1498720785, stop = None, period = 60 * 60 * 24 * 7):
        if data_id == None:
            data_id = self.name
...
class Ic_controller():
    ...
    def import_all_boxes(self):
        for box in self.heat_boxes:
            print "Timeseries:"+"ic-meter." + box['boxId']
...
config = {
    ...
    'ic_user': os.environ.get('IC_USER','hagel@byg.dtu.dk'),
    'ic_password': os.environ.get('IC_PASSWORD','DTUbyg402'),
